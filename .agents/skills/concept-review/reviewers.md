...
採用指摘の確定、重複統合、重大度の確定、品質ゲートの適用、最終判定、findings の作成を行う。議長は新しい指摘を追加しない。3つの `agent_id` と Phase 1/2 の完了を検証できた場合だけ、Chair が `reviews/<コンセプトシートのベース名>-review-findings.md` を作成または上書きする。findings には実行監査情報を記録する。
