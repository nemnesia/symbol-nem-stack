...
Chair は `multi_agent` の `spawn_agent` を3回の別々の呼び出しで実行し、`fork_context: false` と `reasoning_effort: "medium"` を指定して、3つの Reviewer サブエージェントを同じ対象資料に対して並列起動する。`reasoning_effort` は `low` にしない。返された3つの `agent_id` を A/B/C に対応付け、値がすべて存在し相互に異なることを Phase 1 前に検証する。検証できない場合はレビューを中止し、findings を作成しない。
...
コンセプトの一貫性、用語、曖昧さ、スコープ、完全性、論理構成を確認する。コンセプトシートは、非技術者でも目的、課題、提供価値および対象ユーザーを理解できる文章を基本とする。文章表現について、同じ内容の重複やくどさ、理解に必要な説明の不足、要点を隠す過剰な説明、不自然または不要な英語・カタカナ表記、専門用語・略語・技術的な前提知識への過度な依存がないかも確認する。一般に定着した固有名詞・製品名・技術用語を、単に英語やカタカナであるという理由だけで指摘してはならない。技術選定、API、実装、データベース、アーキテクチャは指摘しない。
