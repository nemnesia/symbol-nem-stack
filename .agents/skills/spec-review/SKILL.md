...
- 実装者からの仕様フィードバックはレビュー対象ではなく補助資料として扱う。ユーザーまたは呼び出し元がパスを指定した場合はそのパスを優先し、指定がない場合はリポジトリルートの `.reviews/implementation-spec-feedback.md` が存在するか確認する。存在しない場合は Evidence Used に「未確認」と記録し、レビューを続行する。
